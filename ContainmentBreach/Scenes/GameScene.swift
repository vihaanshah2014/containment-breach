...
final class GameScene: SKScene {
    ...
    private var healthLabel = SKLabelNode(fontNamed: "Menlo")
    private var ammoLabel = SKLabelNode(fontNamed: "Menlo")
    private var waveLabel = SKLabelNode(fontNamed: "Menlo")
    ...
    private func updatePlayer(delta: TimeInterval) {
        ...
        var velocity = desired * player.speed
        if desired.length > 1 { velocity = desired.normalized * player.speed }
    ...
    private func updateEnemies(delta: TimeInterval) {
        for enemy in enemies {
            ...
            let movement = desired * enemy.speed
